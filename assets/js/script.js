...
const offcanvasHeader = function () {
  ...
  if (offcanvasOpen) {
    offcanvasOpen.addEventListener("click", function (e) {
      ...
      if (trigger.dataset.offcanvas != undefined) {
...
const AUTH_USERS_KEY = "fidelityUsers";
const AUTH_CURRENT_USER_KEY = "fidelityCurrentUser";
let authDb = null;

function initAuthDb() {
  if (authDb || typeof window.alasql === "undefined") {
    return;
  }

  try {
    authDb = new alasql.Database();
    authDb.exec("CREATE TABLE IF NOT EXISTS users (id STRING, name STRING, email STRING, password STRING)");
    const raw = localStorage.getItem(AUTH_USERS_KEY);
    if (raw) {
      const users = JSON.parse(raw);
      authDb.exec("DELETE FROM users");
      users.forEach((user) => {
        authDb.exec("INSERT INTO users VALUES (?,?,?,?)", [user.id, user.name, user.email, user.password]);
      });
    }
  } catch (err) {
    console.error("Auth SQL init failed:", err);
    authDb = null;
  }
}
...
function getStoredUsers() {
  initAuthDb();
  if (authDb) {
    return authDb.exec("SELECT * FROM users");
  }
...
function findUserByEmail(email) {
  ...
  if (authDb) {
    const rows = authDb.exec("SELECT * FROM users WHERE LOWER(email) = ?", [normalizedEmail]);
...
function addStoredUser(user) {
  ...
  if (authDb) {
    authDb.exec("INSERT INTO users VALUES (?,?,?,?)", [user.id, user.name, user.email, user.password]);
    saveStoredUsers(authDb.exec("SELECT * FROM users"));
    return;
  }
...
function redirectToLoginIfNeeded() {
  ...
  if (currentUser && (path.endsWith("login.html") || path.endsWith("sign-up.html"))) {
    window.location.href = "admin/dashboard.html";
    return;
  }

  if (!currentUser && path.includes("/admin/")) {
...
function updateAuthUI() {
  ...
  document.querySelectorAll(".header__user--profile__name").forEach((el) => {
  ...
  document.querySelectorAll("a.login__register--link").forEach((el) => {
...
function handleSignupForm() {
  ...
  signupForm.addEventListener("submit", function (event) {
    ...
    const email = emailInput?.value.trim().toLowerCase() || "";
    const password = passwordInput?.value || "";
    const acceptedCondition = conditionInput?.checked;

    if (!name || !email || !password) {
      showAuthMessage("Please fill in name, email, and password to create your account.");
      return;
    }
    ...
    if (!acceptedCondition) {
      showAuthMessage("Please agree to the terms and conditions before creating an account.");
      return;
    }

    const existingUser = findUserByEmail(email);
    if (existingUser) {
      showAuthMessage("An account with this email already exists. Please log in instead.");
      return;
    }
...
function attachLogoutHandler() {
  document.addEventListener("click", function (event) {
    const logoutButton = event.target.closest("[data-auth-logout], .user__profile--log-out__btn");
    if (!logoutButton) {
      return;
    }
    logoutCurrentUser();
